...
final class TimerModel: ObservableObject {
    ...
    private func finished() {
        ...
        let alarmHandle = playAlarm(for: nextPhase)
        
        // 3. Update focus and break counts
        if outgoingPhase == .focus { completedFocusCount += 1 }
        else if outgoingPhase == .shortBreak { completedShortBreakCount += 1 }
        else { completedLongBreakCount += 1 }
        
        // 4. Update the global phase to the nextPhase
        phase = nextPhase
        totalSeconds = seconds(for: phase) // update the new phase seconds
        remaining = totalSeconds // set remaining to new phase seconds
        
        // 4. Present Overlay
        presentOverlay(nextPhase: nextPhase, alarmHandle: alarmHandle) { [weak self] in
            self?.start()
        }
    ...
    private func playAlarm(for nextPhase: Phase) -> AlarmHandle {
        let isFocus = (nextPhase == .focus)
        let url = isFocus ? settings.resolvedFocusSoundURL() : settings.resolvedBreakSoundURL()
        
        if let url = url {
            return AlarmPlayer.play(url: url, duration: settings.alarmDurationSeconds)
        }
        
        // Fallback to legacy sound config
        return AlarmPlayer.playAlarm(
            soundName: isFocus ? settings.focusAlarmSoundName : settings.breakAlarmSoundName,
            fileExtension: isFocus ? settings.focusAlarmFileExtension : settings.breakAlarmFileExtension,
            customSoundPath: isFocus ? settings.focusAlarmCustomSoundPath : settings.breakAlarmCustomSoundPath,
            duration: settings.alarmDurationSeconds
        )
    }
    
    private func presentOverlay(nextPhase: Phase, alarmHandle: AlarmHandle, onStart: @escaping () -> Void) {
        let isFocus = (nextPhase == .focus)
        
        // 1. Check if overlay is enabled in settings
        if isFocus && !settings.focusOverlayEnabled { onStart(); return }
        if !isFocus && !settings.breakOverlayEnabled { onStart(); return }

        // 2. Resolve Image
        let customPath = isFocus ? settings.focusOverlayCustomImagePath : settings.breakOverlayCustomImagePath
        let defaultName = isFocus ? settings.focusOverlayImageName : settings.breakOverlayImageName
        let finalImage = !customPath.isEmpty ? customPath : (!defaultName.isEmpty ? defaultName : nil)
        ...
        if settings.autoCycleEnabled {
            ...
            if isFocus {
                ...
                message = (nextPhase == .longBreak) ? settings.lBreakMessage : settings.sBreakMessage
                 duration = (settings.breakOverlayDurationMode == .fixedSeconds) ? settings.breakOverlaySeconds : nil
                 primaryButton = nil
            ...
            ? settings.focusMessage
                : (nextPhase == .longBreak ? settings.lBreakMessage : settings.sBreakMessage)
            duration = nil
            primaryButton = isFocus ? "Start Focus" : nil
            counterLabel = nil
            showDismiss = isFocus ? true : settings.breakOverlayShowDismissButton
        }
        ...
        OverlayController.shared.show(
            .init(
                colorHex: isFocus ? settings.focusOverlayColorHex : settings.breakOverlayColorHex,
